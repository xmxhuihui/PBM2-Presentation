...
# -*- coding: utf-8 -*-
"""
Created on Sun May 10 19:13:57 2020
...
for k in range(n_trials):
    ...
    u_I=np.sqrt(N_E)*(H_I+W_IE_mean*v_E_mean-np.sqrt(N_I)/np.sqrt(N_E)*W_II_mean*v_I_mean)
    eta_E=np.random.normal(0,1)
    eta_I=np.random.normal(0,1)
    h_E0=(u_E+eta_E*np.sqrt(s_EE2)+eta_I*np.sqrt(s_EI2))
    h_I0=(u_I+eta_E*np.sqrt(s_IE2)+eta_I*np.sqrt(s_II2))
...
# h_II=np.zeros(n_trials)
    ...
    eta_E=np.random.normal(0,1)
    eta_I=np.random.normal(0,1)
    h_E[k]=(u_E+eta_E*np.sqrt(s_EE2)+eta_I*np.sqrt(s_EI2))-h_E0
    h_I[k]=(u_I+eta_E*np.sqrt(s_IE2)+eta_I*np.sqrt(s_II2))-h_I0
# h_IE[i]=(u_rIE_I+eta_E*np.sqrt(s_rIE2)+eta_I*np.sqrt(s_II2))/N_I
# h_II[i]=(u_rII_I+eta_E*np.sqrt(s_IE2)+eta_I*np.sqrt(s_rII2))/N_I

plt.figure()
plt.xlabel('Synaptic Input(mV)')
