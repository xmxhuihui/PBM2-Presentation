...
plt.figure()
plt.plot(range(total_time + 1), v[5], 'b')
plt.show()
plt.figure()
...
sns.distplot(e_firing_rates[0], color='b', kde_kws={'bw': 0.1})
...
plt.xlabel('Firing rate(Hz)')
plt.ylabel('Probability density($Hz^{-1}$)')
sns.distplot(i_firing_rates[0], color='r', kde_kws={'bw': 0.1})
...
plt.plot(x,e_e_corr_Scores,'-k')
plt.scatter(x,e_e_corr_Scores)
plt.plot(x,e_corr_scores,'-b')
plt.scatter(x,e_corr_scores)
plt.plot(x,i_corr_scores,'-r')
plt.scatter(x,i_corr_scores)